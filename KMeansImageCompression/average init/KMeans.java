...
public class KMeans {
    ...
    private static void kmeans(int[] rgb, int k){
    	...
    	//Random rand = new Random(); //for random init
    	
    	//distance threshold and variables for average init
    	int thresh = 65025;
    	int avg = 0;
    	int tally = 1;
    	int index = 0;
    	...
    	for (int i = 0; i < k; i++) {
    		...
    		for (int ii = index; ii < rgb.length; ii++) {
	    		double d = sq_eu_dist(sorted[ii], avg);
	    		if (d > thresh) {
	    			centers[i] = avg;
	    			avg = 0;
	    			index = ii;
