...
train_directory = 'C:\\Users\\brett\\source\\repos\\ML Project 1\\ML Project 1\\project1_datasets\\enron1_train\\enron1\\train'
...
for filename in os.listdir(train_directory):
    ...
    if os.path.isfile(f):
        myf = open('C:\\Users\\brett\\source\\repos\\ML Project 1\\ML Project 1\\project1_datasets\\enron1_train\\enron1\\train\\' + filename, 'r', errors='replace').read()
...
tct = [np.ones(len(w)), np.ones(len(w))]
for i in range(dfl):
    if (df.hamspam[i] == 1):
        tct[0] = tct[0] + df.ber[i]
    else:
        tct[1] = tct[1] + df.ber[i]
...
for i in range(2):
    if i == 0:
        ...
        k = countspam
    for j in range(dfl):
        condprob[i][j] = tct[i][j] / k

condprob[0][0] = float(1) / float(countham)
condprob[0][0] = float(1) / float(countspam)

test_directory = 'C:\\Users\\brett\\source\\repos\\ML Project 1\\ML Project 1\\project1_datasets\\enron1_test\\enron1\\test'
...
for filename in os.listdir(test_directory):
    ...
    if os.path.isfile(f):
        myf = open('C:\\Users\\brett\\source\\repos\\ML Project 1\\ML Project 1\\project1_datasets\\enron1_test\\enron1\\test\\' + filename, 'r', errors='replace').read()
        if (filename.find('ham') > 0):
